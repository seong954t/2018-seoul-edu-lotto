describe("로또 함수 테스트", function(){
    describe("getRandomNum 함수 테스트", function(){
        it("1~45 사이의 수 반환", function(){
    ...
    describe("checkDuplicatedNum 함수 테스트", function(){
        it("([1, 7, 5, 3, 6, 10], 1) 는 중복이 있다.", function(){
            expect(checkDuplicatedNum([1, 7, 5, 3, 6, 10], 1)).toBeTruthy()
        })

        it("([11, 27, 16, 23, 41, 19], 9) 는 중복이 없다.", function(){
            expect(checkDuplicatedNum([11, 27, 16, 23, 41, 19], 9)).toBeFalsy()
        })
    })

    describe("checkDuplicatedNumInList 함수 테스트", function(){
        it("[1, 7, 11, 29, 15, 23]는 중복이 없다.", function(){
            expect(checkDuplicatedNumInList([1, 7, 11, 29, 15, 23])).toBeFalsy()
        })

        it("[11, 23, 20, 11, 35, 42]는 중복이 있다.", function(){
            expect(checkDuplicatedNumInList([11, 23, 20, 11, 35, 42])).toBeTruthy()
        })
    })

    describe("getLottoNums 함수 테스트", function(){
    ...
    describe("getWinningNumbersAndBonusNumber 함수 테스트", function(){
        it("winning_numbers와 bonus_number에 중복 체크", function(){
            getWinningNumbersAndBonusNumber()
            expect(checkDuplicatedNum(winning_numbers, bonus_number)).toBeFalsy()
        })
    })

    describe("getMachedNum 함수 테스트", function(){
        it("winning_numbers [2, 7, 11, 23, 25, 30], lotto_nums [11, 29, 9, 13, 20, 7]의 중복 개수는 2개이다.", function(){
            winning_numbers = [2, 7, 11, 23, 25, 30]
            expect(getMachedNum([11, 29, 9, 13, 20, 7])).toBe(2)
        })
    })

    describe("getWinningResult 함수 테스트", function(){
